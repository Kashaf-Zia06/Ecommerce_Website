...
const Cart = () => {
  const {navigate, updateQuantity,products, currency, cartItem } = useContext(ShopContext)
            ...
            return (
              <div key={index} className='py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4 '>
                ...
                onChange={(e)=>e.target.value===''||e.target.value===0?null:updateQuantity(item._id,item.size,Number(e.target.value))}
                  ...
                  <img className='w-4 sm:w-5 mr-4 cursor-pointer'
                  onClick={()=>{updateQuantity(item._id,item.size,0)}} src={assets.bin_icon} alt="delete-icon" />
